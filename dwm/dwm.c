...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, issticky, isalwaysontop, isterminal, noswallow;
	pid_t pid;
	Client *next;
	Client *snext;
	Client *swallowing;	
...
togglebar(const Arg *arg)
{
	...
	if (usealtbar)
		XMoveResizeWindow(dpy, selmon->traywin, selmon->tx + sp, selmon->by + vp, selmon->tw - 2 * sp, selmon->bh);
	arrange(selmon);
}
